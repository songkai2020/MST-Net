...
class MST_Net(nn.Module):
    def __init__(self, img_size = 256,img_ch=1, output_ch=1,filter_dim=32):
        ...
        self.gate3 = nn.Parameter(torch.ones(1))
        self.gate4 = nn.Parameter(torch.ones(1))
        self.gate5 = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x):
        ...
        #Attention
        x2_ = self.atten2(x2)*self.gate2
        x3_ = self.atten3(x3)*self.gate3
        x4_ = self.atten4(x4)*self.gate4
        x5_ = self.atten5(x5)*self.gate5
